...
**Original file:** `compress-folders-filtered.py` (root, kept for reference)  
...
### Excluded by default
**Directories:** `.venv`, `venv`, `__pycache__`, `.pytest_cache`, `.mypy_cache`, `node_modules`, `dist`, `build`, `out`, `target`, `.next`, `.nuxt`, `.cache`, `.tox`, `.eggs`, `.idea`, `.vscode` (and optionally `.git`).  
**File patterns:** `*.pyc`, `*.pyo`, `*.log`, `*.tmp`, `*.temp`, `*.obj`, `*.o`, `*.class`.
